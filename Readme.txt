...
5. run command 'docker-compose run web python manage.py createsuperuser'
6. run command 'docker-compose up'
7. Verify if the postgres and viabackend_web containers are running using 'docker ps'
...
5. In the docker terminal, type 'docker run -p 80:80 ng-docker-frontend' and click enter
6. Open another docker terminal and type 'docker ps' to verify if the front end docker container is up and running
7. Type 'http://192.168.99.100:80' or 'http://localhost:80' depending on the OS.
